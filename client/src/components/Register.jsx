...
const Register = () => {
  const [vl, setVl] = useState({uname:'', ueml:'', upass:'', uphn:'', ufname:'', uaddr:'', upin:''})
  let name, value
  ...
  const hndlsub = async (e) => {
    ...
    if (!uname || !ueml || !upass || !uphn || !ufname || !uaddr || !upin) {
      window.alert(`All fields are required...!`)
    }
    else {
      try {
        const res = await axios.post(`${server}/user/create`, {uname, ueml, upass, uphn, ufname, uaddr, upin})
